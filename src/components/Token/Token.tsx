...
const Token = ({ token, isOnSale, onTransfer, onBuy, onSale, onMint }: TokenCompProps) => {
  const [transfer, setTransfer] = useState<boolean>(false)
  const [onSaleActive, setOnSale] = useState<boolean>(false)
  const [address, setAddress] = useState<string>('')
  const [price, setPrice] = useState<string>('')
  const { user, ethPrice, contractDetails, transferToken, buyToken, mintToken, withdrawItem, setTokenSale } = useAppState()
  ...
  const onMintClick = (e: MouseEvent) => {
    e.preventDefault()
    onMint && mintToken(token.price, token.name, token.image, user?.address)
  }

  const onWithdrawClick = (e: MouseEvent) => {
    e.preventDefault()
    onMint && withdrawItem(token.image)
  }
  ...
  // console.log("for debug. come to fetchOwner:", owner);
  const { data: owner } = useSWR(token.id, fetchOwner)

  // console.log("for debug. come to Token's token.id:", token.id);
  var {data} = useSWR(`${METADATA_API}/token/${token.id}`, fetcherMetadata)
  ...
  const priceText = onMint?"Mint Price":"Sold Price"
  ...
  return (
    <Card variant="nft">
      ...
      <Box p={3} pt={2}>
        ...
        {onMint &&  (!owner || typeof owner !== 'string') &&(
          <Flex mt={3} sx={{ justifyContent: 'left', width: '100%' }}>
            <Button
              sx={{
                opacity: (owner && typeof owner === 'string')
                  ? 0.5
                  : 1,
                pointerEvents: (owner && typeof owner === 'string')
                  ? 'none'
                  : 'visible',
              }}
              onClick={onMintClick}
